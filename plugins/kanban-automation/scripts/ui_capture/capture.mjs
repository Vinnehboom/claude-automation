...
//
// --config is the project's .claude/ui-capture.json; --project-dir is the
// checkout its commands run in. run.sh supplies both.
//
...
function parseArgs(argv) {
  ...
  for (const key of ['targets', 'out', 'base-url', 'config', 'project-dir']) {
...
// The project supplies its own sign-in surface. Every field is required:
// a half-configured sign-in fails as a browser timeout deep in the run,
// which reads as a page fault rather than as the configuration error it is.
const SIGN_IN_FIELDS = ['path', 'email_selector', 'password_selector', 'submit_selector', 'credentials_command'];

function readSignInConfig(configPath) {
  let raw;
  try {
  ...
  const signIn = raw.sign_in;
  if (!signIn) throw new Error(`${configPath} has no "sign_in" section`);
  const missing = SIGN_IN_FIELDS.filter((field) => !signIn[field]);
  if (missing.length > 0) {
    throw new Error(`${configPath}: "sign_in" is missing ${missing.join(', ')}`);
  }
  return signIn;
}

function adminCredentials(signIn, projectDir) {
  const output = execFileSync('sh', ['-c', signIn.credentials_command], {
    cwd: projectDir,
    encoding: 'utf8',
  });
  ...
  if (!email || !password) {
    throw new Error(`sign_in.credentials_command printed no "email|password" line: ${JSON.stringify(line)}`);
  }
  return { email, password };
}

async function signIn(page, baseUrl, credentials, signInConfig) {
  await page.goto(new URL(signInConfig.path, baseUrl).toString());
  await page.fill(signInConfig.email_selector, credentials.email);
  await page.fill(signInConfig.password_selector, credentials.password);
  await Promise.all([
    page.waitForNavigation({ waitUntil: 'networkidle' }),
    page.click(signInConfig.submit_selector),
  ]);
}
...
function redirectedToSignIn(page, target, signInPath) {
  if (target.path === signInPath) return false;
  try {
    return new URL(page.url()).pathname === signInPath;
  } catch {
    return false;
  }
}
...
async function captureViewport(browser, targets, baseUrl, credentials, signInConfig, outDir, viewportDef, manifest) {
...
async function main() {
  ...
  const signInConfig = readSignInConfig(args.config);
  const credentials = adminCredentials(signInConfig, args['project-dir']);
  ...
  try {
    for (const viewportDef of VIEWPORTS) {
      await captureViewport(browser, targets, args['base-url'], credentials, signInConfig, outDir, viewportDef, manifest);
    }
  } finally {
    await browser.close();
  }
