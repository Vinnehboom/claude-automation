...
## What you hand back
Write the full review — every finding, its file:line, and its failing scenario — to `docs/pipeline-cache/<TASK_ID>/review-round-<N>.md`, **in your own worktree.** You cannot write into the developer's worktree from here — a dispatched agent is sandboxed to the worktree it was given, and a write outside it is refused, not merely discouraged. Do not attempt it and do not treat the refusal as an error to work around.

Hand back: the verdict (APPROVE, or `<n>` BLOCKING / `<m>` NON-BLOCKING), the FULL text of every BLOCKING finding inline (file:line, failing scenario, everything — not a compressed one-liner), a one-line-per-NON-BLOCKING-finding list, and the file's path.
...
**Confirmed 2026-09-13, twice in one generation (H-12, H-15): your own worktree can be cleaned up before the orchestrator's next turn copies the file out**, even on a normal completed run with no error — `docs/pipeline-cache/` is gitignored, so a write there leaves no git-visible change, and worktree cleanup does not distinguish "nothing changed" from "changed something untracked." Do not treat the file as a reliable handoff channel at all; write it anyway (it costs nothing and helps when it does survive), but the message you return is the only copy the orchestrator can count on. When in doubt about the cost tradeoff above, favor putting a little more into the message over trusting the file.
