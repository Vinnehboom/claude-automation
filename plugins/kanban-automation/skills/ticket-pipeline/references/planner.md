...
A `video` target also carries `steps`: an ordered list of `{action, selector, value}` entries, `action` one of `click`, `fill`, `wait_for`, `press` (`value` applies to `fill` and `press` only). Reach for video only when the interaction itself is what a reader needs to see — attaching a modifier, completing a draft pick — not for a page whose final state a still already shows. The evidence page holds every video alongside the stills as an ordinary published file, so a video target carries no size tradeoff against a page-size limit the way an embedded copy would.
