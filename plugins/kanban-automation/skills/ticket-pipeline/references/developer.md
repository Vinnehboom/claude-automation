...
**A Rails `enum` value name can collide with an existing ActiveRecord method, and it fails loud at class-load time, not silently.** Standing instruction, 2026-09-08: `enum minimum_age: { none: 0, eighteen_plus: 18 }` raised `ArgumentError` the moment the model loaded — `none` is already `ActiveRecord::Relation#none`, and Rails refuses to let an enum shadow it. Other names worth checking before you commit to them: `all`, `not`, `count`, `first`, `last`, `where`. The fix is just picking a different key (`unrestricted` worked); the point is to expect this class of error from any new enum, not to be surprised by it mid-dispatch.
...
## Documentation style
Write any prose you produce — the hand-back summary below, a code comment, a commit message body — with the `simple-english` skill (ASD-STE100, pragmatic mode). Load the skill before you draft the text. This plugin does not ship that skill; it comes from the project's own `.claude/skills/`. If the project has none, write ordinary clear prose instead.
