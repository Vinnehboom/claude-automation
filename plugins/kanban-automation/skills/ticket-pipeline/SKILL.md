...
1b. **UI capture (standing instruction, 2026-09-06 — H-8).** After the rebase gate, before marking the PR ready. Run it from the ticket branch's own worktree, not the orchestrator's own checkout — confirm `git rev-parse --abbrev-ref HEAD` matches the ticket's branch before calling `run.sh`. `run.sh` resolves the core surface and captures whatever code is checked out where it runs; running it from the wrong checkout captures the wrong PR and still reports success.
   ...
   **Skip rule first.** If the branch's diff touches nothing under `app/`, `config/`, `db/`, or `lib/`, and doesn't touch `Gemfile` or `Gemfile.lock`, skip the rest of this step, write one line saying why, and go straight to step 2 — with the skip-path replacement text below. Otherwise:
   - Write the ticket's `## Capture plan` section — the developer's amended, final version, already on the card since Phase 2 (see references/developer.md and Phase 2 above) — verbatim to `tmp/ui-capture/<TASK_ID>.json`, in the shape `script/ui_capture/example.json` shows: a page to capture, nothing more. (Video capture is deferred to the evidence-page ticket that replaces chat delivery below — it comes back once that lands.)
   - Run `script/ui_capture/run.sh --ticket <TASK_ID> --targets tmp/ui-capture/<TASK_ID>.json --out tmp/ui-capture/<TASK_ID>`.
