...
1b. **UI capture (standing instruction, 2026-09-06 — H-8).** After the rebase gate, before marking the PR ready. Run it from the ticket branch's own worktree, not the orchestrator's own checkout — confirm `git rev-parse --abbrev-ref HEAD` matches the ticket's branch before calling `run.sh`. `run.sh` resolves the core surface and captures whatever code is checked out where it runs; running it from the wrong checkout captures the wrong PR and still reports success.
   ...
   **Send the evidence, then erase (standing instruction, 2026-09-06 — interim delivery).** Notion attachment is not available: this session's own network proxy denies the direct HTTPS upload `notion-create-attachment` needs (`CONNECT api.notion.com` returns 403), even though the Notion MCP tools themselves work fine over their own route. There is no working fallback path to Notion for the files themselves, so don't attempt one and don't leave upload code or instructions around as something to "try first" — it cannot succeed here.
   - The orchestrator sends every manifest entry whose `attach` field is `true` and whose `file` is not `null` into this session as a chat attachment, so Vinnie has the evidence at the moment he's asked to look at the PR — this excludes a core still already folded into its viewport's contact sheet, and excludes an entry that produced nothing to capture.
   - Then erase `tmp/ui-capture/<TASK_ID>/`. There is no upload to confirm any more, so there is nothing to wait on before erasing, and no failed-upload case that would call for keeping the folder around.
   - This delivery is **interim**. A chat attachment reaches Vinnie, but it does not persist anywhere as a record — it isn't attached to the card, and a later reader of the PR or the ticket has nothing to open. A follow-up ticket is expected to replace this with a per-pull-request evidence page that can hold the files themselves; until that lands, the card's text record (below) is the only durable trace a capture happened.

   **Evidence section.** Append a `## Visual evidence` section to the Notion ticket card: which targets were captured, at which viewports, each file's name and status (the files themselves went to Vinnie as chat attachments, per above — the card names them, it doesn't hold them), and the capture's own outcome — clean, a noted tooling failure, or which page failed and how.
2. **Mark ready for review.** Only once step 1 passes, and step 1b did not block: un-draft the PR (`mcp__github__update_pull_request`, `draft: false`), passing the updated PR body in the SAME call. Replace the `Visual evidence: pending the Gatekeeper.` placeholder line (see "Opening the PR" step 4) with one of:
   - A real link to the card's new `## Visual evidence` section, on a normal capture (whether or not it found something worth noting there).
   - `Visual evidence: not captured (the diff touches no application code).` on the skip-rule path — there is no `## Visual evidence` section to link to, since step 1b never ran the capture.
   - `Visual evidence: capture did not complete (tooling failure) — see the Notion card.` on a tooling failure — the section exists and explains what went wrong, but has no real attachments to point to.
