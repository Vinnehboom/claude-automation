...
4. Open the PR AS A DRAFT (`draft: true`) against `main` (or the dependency branch the plan named — see "Stacked PRs for dependent tickets" below) using the GitHub MCP tools (`mcp__github__create_pull_request`) — never the `gh` CLI, which isn't available in this environment. Title: `<Task ID> — <ticket name>`. Body: what changed and why (from the plan's Goal/Decisions), a link to the Notion ticket card (where the full plan lives — do NOT link a repo file, there isn't one), a one-line note on review outcome, and the line `Visual evidence: pending the Gatekeeper.` on its own — the Gatekeeper (Phase 5, step 1b) replaces that line, in the same call that un-drafts the PR, with a real link, a skip note, or a tooling-failure note (see Phase 5 step 2 for the exact wording of each). No Claude co-author trailer or generated-with footer on the PR body — same override as the commits. Write the body with the `simple-english` skill (ASD-STE100, pragmatic mode) — load it before you draft. This plugin does not ship that skill; it comes from the project's own `.claude/skills/`. If the project has none, write ordinary clear prose instead.
...
1b. **UI capture (standing instruction, 2026-09-06 — H-8).** After the rebase gate, before marking the PR ready. Run it from the ticket branch's own worktree, not the orchestrator's own checkout — confirm `git rev-parse --abbrev-ref HEAD` matches the ticket's branch before calling `run.sh`. `run.sh` resolves the core surface and captures whatever code is checked out where it runs; running it from the wrong checkout captures the wrong PR and still reports success.
   ...
   **These two outcomes are different — do not conflate them, and judge them on the manifest's field values, not on reading the page yourself.**
   - **A failure OF THE CAPTURE never blocks the PR.** `run.sh` exiting non-zero (1 usage error, 2 boot failure, 3 capture failure, 4 budget expired) is a tooling problem, not a finding about the ticket's code. Note it plainly in the evidence section below and continue straight to step 2, using the tooling-failure replacement text. A manifest entry with a `null` status and a non-empty `error` is this same bucket, even when the overall run still exits 0 — a page that failed to load, or one that quietly bounced to the sign-in page instead of rendering, is a tooling failure, not a blocking fault, and it does not carry a status of `200` either.
   - **A capture that SUCCEEDS and produces an entry whose `status` is a number in the 5xx range DOES block.** Leave the PR a draft, leave the Notion card at "In progress," and tell Vinnie which page returned which status, instead of continuing to step 2. This is a real fault the capture found, not a tooling failure, and it needs a human decision before the PR goes further. A 4xx status does not block on its own — `UiCapture::CoreTargets` already excludes a route whose controller doesn't implement the action, so a core-surface 4xx should not occur; if one appears anyway, or a ticket target 4xxs, note it in the evidence section but proceed to step 2.

   **Publish the evidence page (2026-09-14 — H-12, replaces the interim chat delivery H-8 shipped).** Notion still cannot hold the files — the network proxy still denies the direct HTTPS upload `notion-create-attachment` needs — but a published Artifact can hold them directly, as ordinary files next to the page, so this no longer routes through Notion at all. Check the card's `## Visual evidence` section for an existing `Evidence page: <url>` line first.
   ...
   - **Every later run for this ticket** (the card already names a page): `Artifact`, `action: "read"` on that URL first — the tool refuses a publish this session has not read — then republish to the SAME `url` with this round's new files added, and `capabilities` left out so the stored declaration carries forward.
   - **Which files.** Every manifest entry from `result.json` whose `attach` is `true` and whose `file` is not `null`: a contact sheet, a core page that did not answer 200, or a ticket-specific target, still or video. Publish each one under `images/<TASK_ID>-r<round>-<name>.<extension>`, `<round>` being one more than the highest round already on the page (round 1 on the first run). Never repeat an earlier round's file name in a later publish — the Artifact tool keeps a file a publish does not mention, so nothing already on the page is ever lost.
   ...
   **Evidence section.** Append the section on the first run, or update it on every later one — a `## Visual evidence` section on the Notion ticket card. Its first line is always `Evidence page: <url>`, exactly in that form; `kanban-cycle`'s board reads this line to link the ticket's row (see its SKILL.md step 7). Follow it with the round count and one line per round naming that round's outcome. This is a short summary — the page itself is the record now, not this section.
2. **Mark ready for review.** Only once step 1 passes, and step 1b did not block: un-draft the PR (`mcp__github__update_pull_request`, `draft: false`), passing the updated PR body in the SAME call. Replace the `Visual evidence: pending the Gatekeeper.` placeholder line (see "Opening the PR" step 4) with one of:
   - A real link to the evidence page, on every run that reached the capture — the round it just wrote names its own outcome, tooling failure included, so the page is the one place to point at either way.
