...
**This isolation is not optional and not a "best effort." Start each phase as a genuinely separate subagent with the `Agent` tool.** A subagent starts with only the prompt you write. It does not inherit this thread's conversation, so the Reviewer sees the ticket, the cached docs and the diff, and nothing of the developer's reasoning. Past runs (B-2, C-4, C-5, C-17, C-6) substituted an in-session review, and C-17 then drew five design-level review comments that a blind reviewer would have caught. If the `Agent` tool is unavailable, STOP and say that the reviewer phase cannot run with real isolation. Do not substitute an in-session pass.

The ORCHESTRATOR is you: the Project thread that owns this ticket. You own the flow between phases, the three human checkpoints, the PR, and keeping the Notion card in sync. You start each phase and carry artifacts between them.

## Running in a Project thread

This pipeline runs in one Claude Project thread per ticket. The thread is a top-level session with its own container, checkout and test database.

- **Start subagents directly.** Use the `Agent` tool for the Planner, Developer, Reviewer and Curator, with the model the phase names (`model: "opus"` or `"sonnet"`). They work in this thread's checkout. No worktree is needed, and a subagent cannot start its own subagents.
- **Tell each subagent not to call `mcp__hearthbot__*` tools.** Only the thread posts to the user.
- **Checkpoints are in this thread.** Ask with an `ask_decision` card, or with a reply when the answer is free text. A card holds a short context only, so put long content (the plan, the curator proposals) on the Notion card and link it. Continue on the recommended option only where the checkpoint rules below allow it.
- **Progress goes in the status checklist** (`update_status`). Reply only with a result, a blocker, or a question.
- **The PR is yours.** Subscribe to it when it opens (`subscribe_pr_activity`). CI and review events wake this thread, so fix CI and run review rounds when the event arrives. No cycle does it for you.
- **After a merge, tell the other ticket threads.** Find them with `list_thread_sessions`, and send each one with an open PR a `send_message` that `main` moved, so that it rebases its own branch.
- **You cannot start a thread.** For a follow-up ticket, file it on the board. The cycle thread asks the coordinator for its thread.
...
**No Claude trailer, anywhere.** Commits carry no `Co-Authored-By: Claude` line and no session trailer. PR bodies carry no "Generated with Claude Code" footer. This is Vinnie's rule, and it overrides the attribution lines that the Project harness suggests. A harness-required attribution block at the top of a PR body is not a trailer: keep it when the harness requires it.
...
1. As the FIRST action of any pipeline run, fetch every doc phases will need — the ticket card, the Knowledge Base page, the Decisions database, and the Coding Style Guide page plus its Style Rules database — and write each one verbatim to a local cache file under `docs/pipeline-cache/<TASK_ID>/` (`ticket.md`, `context.md`, `decisions.md`, `style-guide.md`). The style guide is two sources into one file; see "Locating Notion context" step 4. Create the directory if needed. Read the page/database pointers from `.claude/knowledge-base.json` and `.claude/coding-style.json` — never hardcode them.
...
4. The Coding Style Guide (pointers in `.claude/coding-style.json`). This is TWO fetches, not one. Fetch the page at `notion_page_url` for the intro and the Change Log. Then query the database at `rules_database.notion_data_source_url` for the rules themselves — one row per rule, the title being the rule and the row's page body the reasoning. Write both into `style-guide.md`: first the rules whose `Hot list` column is ticked, as a short list of titles under a "The rules that keep coming back" heading, then every Active rule grouped by `Category`, each rule followed by its body text. The hot list is a filter on the rows, not a separate document — never transcribe it from anywhere else. A rule without its body is half a rule, and the body is what the developer reads when the one-line rule leaves doubt. LOAD IT ALL ON INITIALIZATION and hand it in full to the developer and reviewer. It supplements repo conventions; where they conflict, the style guide wins. Where it conflicts with tooling like .rubocop.yml, flag it (a curator job), don't silently pick a side. If genuinely missing, fall back to repo conventions and say so.
...
- Checkpoint 1 — planner's questions. Relay gaps to the user, get answers, feed back. Skip only if genuinely none.
  ...
  **Standing instruction, 2026-09-15:** a "gap" that is purely an unknown fact the bracketed-placeholder Decision (2026-09-11, "Ship compliance tickets with bracketed placeholders, not deferrals") already resolves is not a Checkpoint 1 question — draft the placeholder and proceed; Vinnie reviews the result on the card/PR, not before it exists. A genuine fork in what to build or how to word a real decision (which of several valid designs, whether a card stays open vs. spins off a follow-up, a real legal/business judgment call) is not covered by that Decision and still goes to Checkpoint 1 as a real question. When in doubt which bucket a question falls into, ask — this narrows only the pure-unknown-fact case, it does not widen self-approval into judgment calls.
- Checkpoint 2 — plan approval. **Surface the plan's actual content, in full or faithfully summarized section-by-section — not a one-line "plan's ready, approve?"** In a Project thread the full plan is on the Notion card; the reply summarizes it section by section and links the card, and the `ask_decision` card carries only the go/no-go. A plan the human never really reads is not a checkpoint, it's a formality; skimpy surfacing here is how design mistakes (see the reviewer-isolation note above) reach code before anyone who'd object has actually seen them. Call out the **Convention check** section (see references/planner.md Step 2.5) explicitly and by name — any flagged deviation from a default pattern needs its own visible line, not a mention buried in a Decisions bullet — and ask directly whether it's acceptable, don't assume silence on it means yes. Wait for a go before any branch or code.
...
- If the planner's `## Capture plan` section (references/planner.md Step 2.7) says it acted on a `## Capture notes` entry from the Knowledge Base page, the orchestrator removes that entry from the page at this same point — this is the only place a Capture note gets removed; nothing else in this pipeline does it.
...
- Hands back an amended `## Capture plan` target list (see references/developer.md's Step 5). This amended version governs, not the planner's draft: the orchestrator overwrites the Notion card's `## Capture plan` section with it (`notion-update-page`) right away, the same way Phase 1 writes the plan back — so the card always reflects what the Gatekeeper will actually run at Phase 5, step 1b, not what the planner guessed before any of this code existed.
...
4. Open the PR AS A DRAFT (`draft: true`) against `main` (or the dependency branch the plan named — see "Stacked PRs for dependent tickets" below) using the GitHub MCP tools (`mcp__github__create_pull_request`) — never the `gh` CLI, which isn't available in this environment. Title: `<Task ID> — <ticket name>`. Body: what changed and why (from the plan's Goal/Decisions), a link to the Notion ticket card (where the full plan lives — do NOT link a repo file, there isn't one), a one-line note on review outcome, and the line `Visual evidence: pending the Gatekeeper.` on its own — the Gatekeeper (Phase 5, step 1b) replaces that line, in the same call that un-drafts the PR, with a real link, a skip note, or a note that this round's evidence could not be published (see Phase 5 step 2 for the exact wording of each). No Claude co-author trailer or generated-with footer on the PR body — same override as the commits. Write the body with the `simple-english` skill (ASD-STE100, pragmatic mode) — load it before you draft. This plugin does not ship that skill; it comes from the project's own `.claude/skills/`. If the project has none, write ordinary clear prose instead.
...
**The Gatekeeper's UI capture (Phase 5, step 1b) is not this phase and does not stand in for it.** It screenshots the app's own seeded demo data, signed in as the demo admin account — not a real user exercising the ticket's done-criteria against a live deploy. Re-enabling Phase 4 needs its own evidence path; it must not fold into or duplicate step 1b's Notion attachments.
...
**Reduced scope, 2026-08-29:** the CI gate and testing-evidence gate that used to be steps 1–2 here are dropped, alongside disabling Phase 4 (Tester) — there's no live-verification evidence to gate on, and CI-red is still this session's job to fix, just via this environment's standing "drive a PR you opened to green" duty (see repo-level instructions) rather than as an explicit step in this skill. That duty is not optional just because it's no longer written here — a red or conflicted PR this pipeline opened is still this session's to fix, same as any other PR it owns. What remains below is git hygiene, plus the UI capture added 2026-09-06 (step 1b): keep the branch current, capture the app's visual surface, then say go.
...
1b. **UI capture (standing instruction, 2026-09-06 — H-8).** After the rebase gate, before marking the PR ready. Run it from the ticket branch's checkout — confirm `git rev-parse --abbrev-ref HEAD` matches the ticket's branch before calling `run.sh`. `run.sh` resolves the core surface and captures whatever code is checked out where it runs; running it from the wrong checkout captures the wrong PR and still reports success.

   **Skip rule first.** If the branch's diff touches nothing under `app/`, `config/`, `db/`, or `lib/`, and doesn't touch `Gemfile` or `Gemfile.lock`, skip the rest of this step, write one line saying why, and go straight to step 2 — with the skip-path replacement text below. Otherwise:
   - Write the ticket's `## Capture plan` section — the developer's amended, final version, already on the card since Phase 2 (see references/developer.md and Phase 2 above) — verbatim to `tmp/ui-capture/<TASK_ID>.json`, in the shape `${CLAUDE_PLUGIN_ROOT}/scripts/ui_capture/example.json` shows: a page to capture, or a flow to record, nothing more.
   - Run `${CLAUDE_PLUGIN_ROOT}/scripts/ui_capture/run.sh --ticket <TASK_ID> --targets tmp/ui-capture/<TASK_ID>.json --out tmp/ui-capture/<TASK_ID>`. The driver ships with this plugin; the project supplies `.claude/ui-capture.json` (how to boot, how to list its own routes, how to sign in). If that file is absent the run exits 1 saying so — that is a project that has not been wired for capture yet, so treat it as the skip path below, not as a fault in the ticket.
   - Read `tmp/ui-capture/<TASK_ID>/result.json`.

   **Read the manifest's field values to find the round's outcome — do not conflate the three, and do not decide from reading the page yourself.**
   - **`tooling_failure`** — `run.sh` exiting non-zero (1 usage error, 2 boot failure, 3 capture failure, 4 budget expired) is a tooling problem, not a finding about the ticket's code, and so is a manifest entry with a `null` status and a non-empty `error` even when the overall run exits 0 — a page that failed to load, or one that quietly bounced to the sign-in page instead of rendering, is a tooling failure, not a blocking fault, and it does not carry a status of `200` either.
   - **`blocked`** — a capture that SUCCEEDS and produces an entry whose `status` is a number in the 5xx range. A 4xx status does not block on its own — `UiCapture::CoreTargets` already excludes a route whose controller doesn't implement the action, so a core-surface 4xx should not occur; if one appears anyway, or a ticket target 4xxs, that's `clean`, not `blocked`.
   - **`clean`** — anything else.

   Publish the evidence page below for all three outcomes — the page is the record of what happened, `blocked` and `tooling_failure` included. Only `blocked` stops you from reaching step 2: leave the PR a draft, leave the Notion card at "In progress," and tell Vinnie which page returned which status, instead of continuing. `tooling_failure` does not block — continue to step 2 once the page is published.

   **Publish the evidence page (2026-09-13 — H-12, replaces the interim chat delivery H-8 shipped).** Notion still cannot hold the files — the network proxy still denies the direct HTTPS upload `notion-create-attachment` needs — but a published Artifact can hold them directly, as ordinary files next to the page, so this no longer routes through Notion at all. Check the card's `## Visual evidence` section for an existing `Evidence page: <url>` line first.
   - **Find the next round number.** `Artifact`, `action: "read_db"`, `db_op: "list"`, `collection: "rounds"`, on the page's URL (skip this on the first run — there is no URL yet, and the round is 1). The next round is one more than the highest `round` field among the returned documents. Do this before naming any file below — a round number guessed from anywhere else (the page's HTML, or memory of an earlier turn) can repeat an existing round and `set` over it, and `write_db`'s `set` is a full replace with no undo. Write `round` into the document as a JSON number, never a numeric string — the page's own `orderBy("round")` sorts on the field's real type, and a string sorts round 10 before round 2.
   - **First run for this ticket** (the card names no evidence page yet): read `${CLAUDE_PLUGIN_ROOT}/skills/ticket-pipeline/evidence-page/page.html`, replace its one `{{TICKET}}` placeholder with the real ticket id, and publish it (`Artifact`, `action: "publish"`, `capabilities: {db: {}}`, a `favicon`, an `icon`, `title` left to the file's own `<title>` tag) together with this round's files (see below).
   - **Every later run for this ticket** (the card already names a page): `Artifact`, `action: "read"` on that URL first — the tool refuses a publish this session has not read — then republish to the SAME `url` with this round's new files added, and `capabilities` left out so the stored declaration carries forward. This `read` is required for the publish call itself; it is separate from, and does not substitute for, the `read_db` `list` above.
   - **Which files.** Every manifest entry from `result.json` whose `attach` is `true` and whose `file` is not `null`: a contact sheet, a core page that did not answer 200, or a ticket-specific target, still or video. Publish each one under `images/<TASK_ID>-r<round>-<name>.<extension>`. Never repeat an earlier round's file name in a later publish — the Artifact tool keeps a file a publish does not mention, so nothing already on the page is ever lost.
   - **Write the round.** One `write_db`, `db_op: "set"`, `collection: "rounds"`, `doc_id: "<round>"`, matching the shape in `evidence-page/README.md`: the round number, the time, the pull request's number and URL, the branch, the pushed commit this gate is running against, the PR's check-suite state if known, this step's own outcome (`clean`, `tooling_failure`, or `blocked`), and one `files` entry — `{name, viewport, kind, path, status}` — for each file just published. A round that follows a finished review also carries `findings: {blocking: [...], non_blocking: [...]}`, the reviewer's own finding text, unedited.
   - On the first run only, also `write_db`, `db_op: "set"`, `collection: "meta"`, `doc_id: "info"`: `{ticket: <TASK_ID>, repo: <this repository's full name>}`.
   - **Video size.** A video target records the desktop viewport only (`capture.mjs` enforces this). The Artifact tool still caps a single file at 15 MB and a published version at 64 MB total, earlier rounds' files included. If a publish is rejected for exceeding a cap, do not retry with a smaller file and do not drop an earlier round's files to make room: on a later round for a ticket that already has a page, treat this round's outcome as `tooling_failure` and write the round document without the files that would not fit (note the omission in the round's own text). On a first run, there is no page yet to write a round document to — note the failure directly in the card's evidence section instead, with no page link, and try again on the ticket's next round. Either way, say so in the PR body too (see step 2's third bullet). A ticket that plans more than one or two video targets should raise the budget at Checkpoint 1, before it reaches this limit.
   - Erase `tmp/ui-capture/<TASK_ID>/` once the publish above succeeds. There is nothing left to hold onto it for.

   **Evidence section.** Append the section on the first run, or update it on every later one — a `## Visual evidence` section on the Notion ticket card. Its first line is `Evidence page: <url>`, exactly in that form, whenever a page exists; `kanban-cycle`'s board reads this exact line to link the ticket's row (see its SKILL.md step 7). Follow it with the round count and one line per round naming that round's outcome. On a first-run publish failure there is no URL yet — write one line saying the publish failed and why, instead of the `Evidence page:` line, so this round doesn't look like a page that simply has nothing in it. This section is a short summary either way — the page itself is the record now, not this section.
2. **Mark ready for review.** Only once step 1 passes, and step 1b's outcome is not `blocked`: un-draft the PR (`mcp__github__update_pull_request`, `draft: false`), passing the updated PR body in the SAME call. Replace the `Visual evidence: pending the Gatekeeper.` placeholder line (see "Opening the PR" step 4) with one of:
   - A real link to the evidence page, when this round's publish succeeded (outcome `clean` or `tooling_failure` — both still publish a page; only `blocked` never reaches this step).
   - `Visual evidence: not captured (the diff touches no application code).` on the skip-rule path — step 1b never ran, so this ticket has no evidence page.
   - `Visual evidence: capture ran, but this round's evidence could not be published — see the Notion card.` when the publish itself failed (for example, the size cap above), on the first run or a later one.

   Set the Notion card Status → Review, and tell Vinnie it's actually ready now — this is the real "sending it off for your review" moment, not PR creation. Note plainly that this no longer implies CI was confirmed green by this skill itself — say what CI's actual state is if you know it.
...
4. Tell every other ticket thread with an open PR that `main` moved (see "Running in a Project thread"). Each one rebases its own branch.
5. Run Phase 6 (Curator) against the merged result, then Checkpoint 3.
