...
`dashboard_artifact_url`). If it's missing, say so and
...
- **Vinnie commented/reviewed "lgtm" (or a clear equivalent — "looks
  ...
  **This "don't wait for the next scheduled firing" principle is not scoped
  to a merge inside a `/kanban-cycle` run — it's general.** Standing
  instruction, 2026-09-10: whenever room opens up for a new ticket and this
  session notices it — mid-cycle (as above), mid-conversation with Vinnie,
  right after a PR merges, after triaging a review-fix pass, anywhere — pick
  up the next ready ticket right then. Steps 1/4/5/6 are the check to run,
  not a ritual gated on a Routine firing. A scheduled cycle is a guaranteed
  floor of attention, not the only moment new work is allowed to start.
...
**Eleven more environment facts, learned 2026-08-29 through 2026-09-25, worth knowing before they cost a cycle:**
...
- **`db:prepare`/`db:schema:load` can dirty `db/schema.rb` with a DIFFERENT open PR's not-yet-merged migration, in either a dispatched worktree or the orchestrator's own main checkout.** Confirmed three times in one generation (2026-09-10): twice in dispatched worktrees, once in the orchestrator's own checkout, discovered via a Stop-hook uncommitted-changes warning with local `HEAD` still correctly matching `origin/main`. The shared test database already has one open PR's migration applied; something in the session-start hook's `db:prepare`/`db:schema:load` step picks up that already-migrated state (or the migration file itself) and writes it into `db/schema.rb` in a checkout whose own branch never touched it — a phantom diff. **Before ever committing `db/schema.rb`, run `git diff db/schema.rb` and check the schema version / new columns against what THIS ticket's own migrations actually add** — a mismatch means discard the hunk (`git checkout -- db/schema.rb`), it's contamination, not real work. This applies equally to the orchestrator's own checkout: never commit or push a `db/schema.rb` change there without this same check, and never treat a Stop-hook "uncommitted changes, please commit and push" nudge as instruction to push blindly — investigate first, per this environment's own general git-safety rule.
- **`git worktree add -b <new-branch> origin/<other-ticket's-branch>` silently sets the new branch's upstream to that OTHER branch**, not to a same-named branch on origin. Confirmed 2026-09-10 (P-6, stacked on P-3's branch): a dispatched agent building a stacked ticket branched off another open PR's branch as instructed, and the worktree command set `origin/feat/P-3-...` as the new branch's upstream — so a bare `git push` from that worktree would have pushed the new ticket's commits onto the OTHER ticket's PR. The agent caught it by checking `git status`'s tracking-branch line before its first push and running `git branch --set-upstream-to=origin/<own-branch-name>` (after first pushing with `git push -u origin <own-branch-name>` to create it) to fix it. **Any dispatch instructed to branch off another ticket's branch (stacked PRs) must check its upstream before its first push**, not assume `git worktree add -b` set it to something sane.
- **The classifier blocks rewriting an already-pushed commit's message, not only its authorship — and this holds from the orchestrator's own session directly, not only inside a dispatched agent.** Confirmed 2026-09-25: a cherry-pick onto a temporary branch that carried an amended commit message was refused as `[Git Destructive]`. A plain, read-only `git status` bundled into the same compound Bash command was refused along with it — the classifier does not partially allow a compound command, so a read-only check that needs to run regardless must go in its own separate call, not chained after a risky one.
- **A small `tail_lines` on `get_job_logs` can land on the Postgres service container's teardown log, not the RSpec summary.** Confirmed 2026-09-25: the run summary and the list of failed examples typically sit roughly 130 to 200 lines from the end of the full job log. Request enough lines (or read the whole log) rather than assuming a short tail reaches the actual test output — a short tail read this way looked complete but contained no test information at all.
...
session or Project thread that owns the work reports them.

**One board serves every project.** The same artifact URL goes in every
project's `kanban-cycle.json`. Each project's orchestrator writes only
its own document, keyed by `project_key`. See "Adding a project" at the
end of this step.

The page holds no state. It renders what the orchestrators last wrote to
the artifact's document store, so **one `write_db` call is the entire
update**. Do not republish the HTML.

**This step runs on every cycle, including a quiet one.** A board that is
current only after an eventful cycle cannot be trusted. The page marks a
project whose snapshot is older than eight days with "A cycle was
missed". The one case that skips this step is a cycle that stops early
at step 0 for `/handoff`.

### What to write

One `Artifact` call, `action: "write_db"`, `db_op: "set"`, the configured
`url`, collection `state`, document `<project_key>`. It is a full replace,
so send the whole document every time:
...
- `throughput` — `{weeks: [{week, total, ticket}], ticket_median_hours,
  ...
  ticket-linked ones. Recompute it from merged PRs when a PR merges this
  cycle. Otherwise read the stored document first (`read_db`, `db_op:
  "get"`) and carry `throughput` forward unchanged. The page fills weeks
  with no merges itself, so do not write zero weeks.

Older snapshots also hold `needs_you`, `prs`, `dispatches`, `agents`,
`knowledge`, `evidence`, `caps`, `health`, and `orchestrator`. The page
ignores them. Drop them on the next full replace. Do not write
`cycles/`, `answers/`, `requests/`, or `retractions/` documents: the
page no longer reads or writes them.

**Never publish a new artifact for this board.** Vinnie's bookmark, and
every project that writes to the board, point at
...
with the cycle. Do not invent a replacement board.
...
asked for. Step 7 has already written the board counts to the dashboard.
...
own line, so board progress is one click away from the summary.
...
- Write to `dashboard_artifact_url`; never publish a new artifact for
  the board. A new URL strands the page Vinnie has bookmarked.
- Write only this project's own dashboard document, `state/<project_key>`.
  Another project's orchestrator owns the rest, and overwriting it
  silently erases that project's state.
