...
| `Vinnehboom/claude-automation` | The `kanban-projects` plugin (and `kanban-automation`, the older orchestrator version) | A PR there. Attach the repo with `add_repo` first. |
...
A skill change here reaches every project that installs this plugin.
Projects that still run the generational orchestrator install
`kanban-automation` instead, so a change here does not reach them. A lesson about one project belongs in that project's
...
Run /kanban-projects:ticket-pipeline <Task ID>.
...
1. Makes sure that `kanban-projects:*` skills are loaded. If they are
