...
Every PR that touches `app/`, `config/`, `db/`, or `lib/` gets its core surface captured automatically by the Gatekeeper: every page-rendering route, filled from seeded demo data, as a still image, at two viewports (the project's own `core_targets_command`, named in `.claude/ui-capture.json`). Your `## Capture plan` section is for a ticket-specific page or flow worth its own close-up instead of being folded into the automatic pass's contact sheet. Each entry follows the plugin's `scripts/ui_capture/example.json` shape: `name`, `kind` (`still` or `video`), and `path`. A still target whose `path` matches a core page replaces it, so it gets its own full-size file instead of a spot in the grid.

A `video` target also carries `steps`: an ordered list of `{action, selector, value}` entries, `action` one of `click`, `fill`, `wait_for`, `press` (`value` applies to `fill` and `press` only). Reach for video only when the interaction itself is what a reader needs to see — attaching a modifier, completing a draft pick — not for a page whose final state a still already shows. A video records the desktop viewport only, never mobile. It publishes as an ordinary file next to the evidence page, so it is bound by the Artifact tool's own limits, not a rendered page's: 15 MB for this one file, and 64 MB total for everything the page has published so far, earlier rounds included. Keep a Capture plan to one or two video targets; naming more than that is a Checkpoint 1 question, not something to decide alone.
