...
A `video` target also carries `steps`: an ordered list of `{action, selector, value}` entries, `action` one of `click`, `fill`, `wait_for`, `press` (`value` applies to `fill` and `press` only). Reach for video only when the interaction itself is what a reader needs to see — attaching a modifier, completing a draft pick — not for a page whose final state a still already shows. A video records the desktop viewport only, never mobile. It goes to the repository's `evidence` branch, and the PR body links it: a PR body cannot play a video from a repository, so a reader must open it. GitHub refuses a file over 100 MB. Keep a Capture plan to one or two video targets; naming more than that is a Checkpoint 1 question, not something to decide alone.
