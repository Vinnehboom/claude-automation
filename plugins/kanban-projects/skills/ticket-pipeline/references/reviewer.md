...
## What you hand back
Write the full review — every finding, its file:line, and its failing scenario — to `docs/pipeline-cache/<TASK_ID>/review-round-<N>.md` in the thread's checkout.
...
The split is about cost. Whatever you return stays in the thread's context for the rest of the ticket, so NON-BLOCKING findings stay in the file. BLOCKING findings go inline in full, because they are what decides whether the PR is safe, and the message is the copy the thread can always count on.
